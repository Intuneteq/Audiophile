import React from "react";

import { Images } from "../../Constants";
import "./AudioAd.scss";

const AudioAd = () => {
  return (
    <div className="audio">
      <div className="audio-content">
        <h1>
          BRINGING YOU THE <span>BEST </span>AUDIO GEAR
        </h1>
        <p>
          Located at the heart of New York City, Audiophile is the premier
          store for high end headphones, earphones, speakers, and audio
          accessories. We have a large showroom and luxury
          demonstration rooms available for you to browse and
          experience a wide range of our products. Stop by our store to
          meet some of the fantastic people who make Audiophile the
          best place to buy your portable audio equipment.
        </p>
      </div>
      <picture className="audio-img">
          <source media="(max-width: 550px)" srcSet={Images.audiogearmobile} />
          <source media="(max-width: 900px )" srcSet={Images.audiogeartab} />
          <img src={Images.audiogear} alt="Audiogear" />
        </picture>
    </div>
  );
};

export default AudioAd;
